...
// Send the user's location to the server to retrieve location-based posts
 fetch(`/frentals/forms/get_posts.php?city=${city}`)
 .then(response => response.json())
 .then(posts => {
  //  const postsContainer = document.getElementById('posts');
   posts.forEach(post => {
    ...
    const postsContainer = document.getElementById('posts');
     ...
     <div id="postinner" class="w3-padding-16 w3-round-xlarge w3-margin w3-hover-shadow w3-card w3-mobile">

       <img style="width=100%;"  class="w3-image w3-round w3-border w3-padding" src="  ${post.image} "
       onerror="this.style.display='none'" > </img><br><br>  
       <p class="w3-container">${post.content}</p><br><br>
     <p class="w3-container">Location: ${post.city}</p>
     <p class="w3-container"> ${post.date}</p><br><br>
     <button class="w3-button w3-block w3-dark-grey w3-round-xlarge">+ Connect</button>
</div>
     `;
